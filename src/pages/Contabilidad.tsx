...
import { ArrowDownRight, ArrowUpRight, Receipt, TrendingUp, Edit2, Check, X, Trash2, Landmark, Download, Plus, Sparkles, FileDown } from "lucide-react";
...
export function Contabilidad() {
  ...
  return (
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-8">
      {/* Header */}
      <motion.div variants={itemAnim} className="flex flex-col lg:flex-row justify-between gap-4 md:p-6 relative z-10">
        <div>
          <div className="flex items-center gap-2 text-[#38bdf8]/60 mb-1">
            <Landmark className="w-4 h-4" />
            <span className="text-[10px] font-mono font-bold uppercase tracking-[0.2em] opacity-70">Finanzas</span>
          </div>
          <h1 className="text-xl font-display font-medium text-white tracking-[0.1em] uppercase" >Contabilidad</h1>
          
        <div className="flex flex-col gap-3">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mt-6">
            <div className="flex crystal-panel p-1 rounded-sm w-fit border border-[#38bdf8]/20">
              {[
                { id: 'All', label: 'Todo' },
                ...
                <button 
                  ...
                  className={cn(
                    "px-6 py-2 rounded-sm text-[10px] font-mono font-bold uppercase tracking-[0.2em] transition-all relative overflow-hidden",
                    filterType === tab.id ? "bg-[#38bdf8]/10 text-[#38bdf8]" : "text-[#38bdf8]/40 hover:text-[#38bdf8]/80"
                  )}
                >
                  <span>{tab.label}</span>
                  {filterType === tab.id && (
            ...
            <input 
              ...
              className="w-full sm:w-[320px] px-4 py-2 crystal-input text-xs font-mono transition-all placeholder:text-[#38bdf8]/30 focus:ring-[#38bdf8]/50"
            />
          </div>
          ...
          <div className="flex flex-wrap items-center gap-4 mt-2">
            ...
            >
              ...
              <span className="text-[10px] font-mono font-bold uppercase hidden sm:inline tracking-widest">Exportar</span>
            </button>
      ...
      {/* KPIs */}
      <motion.div variants={itemAnim} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:p-6 relative z-10">
        {[
          { label: "Balance General", value: balanceCaja, icon: Landmark, color: "text-[#38bdf8]", bg: "bg-[#38bdf8]/10", shadow: "" },
          { label: "Total Ingresos", value: totalIngresos, icon: ArrowUpRight, color: "text-[#10b981]", bg: "bg-[#10b981]/10", shadow: "" },
          { label: "Total Egresos", value: totalEgresos, icon: ArrowDownRight, color: "text-[#ef4444]", bg: "bg-[#ef4444]/10", shadow: "" },
          { label: "Ticket Promedio", value: promedioVentas, icon: TrendingUp, color: "text-[#8b5cf6]", bg: "bg-[#8b5cf6]/10", shadow: "" }
        ].map((stat, i) => (
          <div key={i} className={cn("crystal-card p-6 border border-white/5 group overflow-hidden relative", stat.shadow)}>
            <div className={cn("absolute -top-10 -right-10 w-24 h-24 rounded-full blur-[40px]", stat.bg)} />
            <div className="absolute top-0 right-0 p-4 opacity-10 transform scale-150 group-hover:scale-[1.7] transition-transform duration-700 pointer-events-none">
               <stat.icon className={cn("w-10 h-10", stat.color)} />
            </div>
            <h3 className={cn("text-[9px] font-mono font-bold uppercase tracking-[0.2em] mb-4", stat.color)}>{stat.label}</h3>
            <p className="text-xl font-display font-medium text-white tracking-[0.1em]" >{formatCurrency(stat.value)}</p>
            <div className="mt-4 flex items-center gap-2">
               <div className={cn("w-1.5 h-1.5 rounded-full animate-pulse", stat.color.replace('text-', 'bg-'))} />
               <span className="text-[8px] font-mono font-bold uppercase text-[#f8fafc]/40 tracking-widest">Tiempo Real</span>
            </div>
          </div>
        ))}
      </motion.div>

      <motion.div variants={itemAnim} className="mt-4 flex flex-col gap-4 relative z-10">
        <div className="flex items-center gap-2 px-6">
          <Receipt className="w-4 h-4 text-[#38bdf8]/60" />
          <h2 className="text-xl font-display font-medium text-white tracking-[0.1em] uppercase" >Transacciones</h2>
        </div>
              ...
              return (
                <motion.div 
                  ...
                  className={cn(
                    ...
                    isEditing ? "border-[#38bdf8]/50 bg-[#38bdf8]/10" : "hover:bg-white/5 hover:border-[#38bdf8]/20"
                  )}
                >
                  <div className="flex items-center gap-4 min-w-[140px]">
                    <div className={cn(
                      "w-12 h-12 rounded-sm flex items-center justify-center border shrink-0",
                      tx.type === 'Ingreso' ? "bg-[#10b981]/10 text-[#10b981] border-[#10b981]/30" : "bg-[#ef4444]/10 text-[#ef4444] border-[#ef4444]/30"
                    )}>
                    ...
                    {isEditing ? (
                      <input 
                        ...
                        className="crystal-input px-3 py-1.5 text-xs w-full font-mono"
                      />
                    ) : (
                      <div className="flex flex-col">
                        <span className="font-mono font-bold text-[#f8fafc] text-[11px] uppercase tracking-widest">
                    ...
                    {isEditing ? (
                      <div className="flex-1 flex gap-2">
                        <input 
                          ...
                          className="crystal-input px-3 py-1.5 text-sm w-full font-mono font-bold"
                        />
                        <select 
                          value={editForm.category || tx.category}
                          onChange={(e) => setEditForm({...editForm, category: e.target.value as Transaction['category']})}
                          className="crystal-input px-3 py-1.5 text-sm font-mono font-bold bg-[#09090b]"
                        >
                          <option value="Venta">Venta</option>
                          <option value="Compra Proveedor">Compra Prov.</option>
                          <option value="Logística">Logística</option>
                      ...
                      <div className="flex flex-col flex-1 min-w-[200px]">
                        <span className="font-sans font-medium text-[11px] text-white uppercase tracking-[0.1em]"  title={tx.description}>{tx.description}</span>
                        <div className="flex items-center gap-2 mt-1">
                          <span className="text-[9px] font-mono font-bold uppercase tracking-[0.2em] bg-white/5 border border-white/20 px-2 py-0.5 rounded-sm text-[#f8fafc]/70">
                            {tx.category}
                          </span>
                          <span className="text-[9px] font-mono font-bold uppercase tracking-[0.2em] text-[#38bdf8]/70">
